...
import { fetchBills, billTotals } from "@/lib/records";
  ...
  const stats = [
    {
      ...
      value: new Set(list.map((a) => a.email)).size,
    ...
    },
    {
      icon: DollarSign,
      label: "Total Revenue",
      value: `₹${bills.reduce((sum, b) => sum + billTotals(b).total, 0).toFixed(0)}`,
    ...
    <div className="min-h-screen p-4 sm:p-8">
      <header className="glass rounded-2xl p-4 flex items-center justify-between mb-6 shadow-soft">
        <Link to="/" className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-xs text-muted-foreground">Doctor Dashboard</div>
          </div>
        </Link>
        <Button
          variant="outline"
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {stats.map((s, i) => (
          ...
          >
            <div
              className={`w-11 h-11 rounded-xl bg-gradient-to-br ${s.color} flex items-center justify-center mb-3`}
            >
...
function AppointmentRow({ a, onStatus, onReschedule }) {
  return (
    <div className="rounded-xl border bg-background/40 p-4 flex flex-col lg:flex-row lg:items-center gap-4 hover:shadow-soft transition">
      <div className="flex-1 grid sm:grid-cols-2 lg:grid-cols-4 gap-2 text-sm">
        ...
        <div>
          <div className="text-muted-foreground text-xs">Treatment</div>
        ...
        <div>
          ...
          <div className="font-medium">
            {a.status === "Rescheduled" ? (
              <span className="text-purple-600 line-through mr-2 opacity-50">
                {a.date} {a.time}
              </span>
            ) : null}
            {a.status === "Rescheduled"
        ...
        {a.status === "Pending" && (
          <>
            <Button
              ...
              variant="outline"
              title="Approve"
              onClick={() => onStatus(a._id, "Approved")}
            >
              <Check className="w-4 h-4" />
            </Button>
            <Button size="sm" variant="outline" title="Reschedule" onClick={() => onReschedule(a)}>
            ...
            <Button
              ...
              variant="outline"
              title="Reject"
              onClick={() => onStatus(a._id, "Rejected")}
            >
              <X className="w-4 h-4" />
            </Button>
          </>
        ...
        {a.status === "Approved" && (
          <>
            <Button
              ...
              onClick={() => onStatus(a._id, "Completed")}
            >
