...
export default function ReceptionistPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-accent/30">
      ...
      <div
        className="blob w-[400px] h-[400px] bg-secondary -bottom-32 -left-32 animate-float opacity-40"
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-muted-foreground mt-1">
            Create prescriptions and bills, then print or share with the patient.
          </p>
        </motion.div>
...
function PrescriptionForm({ onCreated }) {
  ...
  return (
    <Card className="glass p-6 sm:p-8 rounded-2xl shadow-soft">
      <h2 className="text-xl font-bold flex items-center gap-2">
        <FileText className="w-5 h-5 text-primary" /> New Prescription
      </h2>
      <form onSubmit={submit} className="mt-6 space-y-6">
        ...
        <Field label="Advice / Instructions">
          <Textarea
            ...
            placeholder="Brush twice daily, avoid cold drinks..."
          />
        </Field>
...
function BillForm({ onCreated }) {
  ...
  const update = (i, k, v) =>
    setItems(
      items.map((it, idx) =>
        idx === i ? { ...it, [k]: k === "description" ? v : Number(v) || 0 } : it,
      ),
    );
  ...
  const totals = useMemo(
    () => billTotals({ items, discount: form.discount, taxPct: form.taxPct }),
    [items, form.discount, form.taxPct],
  );
  ...
  return (
    <Card className="glass p-6 sm:p-8 rounded-2xl shadow-soft">
      <h2 className="text-xl font-bold flex items-center gap-2">
        <Receipt className="w-5 h-5 text-secondary" /> New Bill / Invoice
      </h2>
      <form onSubmit={submit} className="mt-6 space-y-6">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <Field label="Patient Name *">
        ...
        <div>
          ...
          <div className="space-y-3">
            {items.map((it, i) => (
              <div
                key={i}
                className="grid grid-cols-1 sm:grid-cols-12 gap-2 items-start p-3 rounded-xl bg-accent/40"
              >
                <Input
                  className="sm:col-span-6"
                  placeholder="Description (e.g. Root Canal)"
                ...
                />
                <div className="sm:col-span-1 text-sm font-medium pt-2">
...
function RecordsList({ title, items, empty, render, onPrint, onDelete }) {
            ...
            return (
              <li key={i._id} className="flex items-center justify-between gap-3 py-3">
                ...
                <div className="flex gap-1 shrink-0">
                  <Button size="sm" variant="outline" onClick={() => onPrint(i)}>
...
function PrintShell({ children, onBack }) {
  ...
  return (
    <div className="min-h-screen bg-muted/40 py-6 print:bg-white print:py-0">
      <div className="max-w-4xl mx-auto px-4 print:px-0 print:max-w-none">
        ...
        <div
          ref={ref}
          className="bg-white text-black rounded-xl shadow-soft p-8 sm:p-10 print:shadow-none print:rounded-none print:p-10"
        >
          {children}
        </div>
      </div>
  ...
  return (
    <div className="border-b-2 border-sky-500 pb-4 mb-6 flex items-start justify-between gap-4">
      ...
      <div className="text-right text-xs text-gray-700 leading-relaxed">
        ...
        <div>
          {CLINIC.website} · Reg: {CLINIC.regNo}
        </div>
      </div>
    </div>
  );
    ...
    <PrintShell onBack={onBack}>
      ...
      <div className="flex justify-between text-sm mb-4">
        ...
        <div className="text-right">
          ...
          {rx.nextVisit && (
            <div>
              <strong>Next Visit:</strong> {rx.nextVisit}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm bg-sky-50 rounded-lg p-3 mb-5">
        ...
        <div>
          ...
          <div className="font-medium">{rx.phone || "—"}</div>
        </div>
        <div>
...
function PrintBill({ bill, onBack }) {
    ...
    <PrintShell onBack={onBack}>
      ...
      <div className="flex justify-between text-sm mb-4">
        <div>
          ...
          <div>
            <strong>Invoice No:</strong> {bill.invoiceNo}
          </div>
          <div>
      ...
      <div className="flex justify-end mt-4">
        <div className="w-full sm:w-72 text-sm space-y-1">
          ...
          <div className="flex justify-between border-t pt-2 font-bold text-base">
            <span>Total</span>
            <span className="text-teal-700">₹{t.total.toFixed(2)}</span>
          </div>
