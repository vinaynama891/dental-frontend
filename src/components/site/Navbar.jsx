...
export function Navbar() {
  ...
  return (
    ...
    >
      <nav
        className={`mx-auto max-w-7xl px-4 sm:px-6 ${scrolled ? "glass shadow-soft rounded-2xl mx-4 sm:mx-6" : ""}`}
      >
        <div className="flex items-center justify-between h-14">
          ...
          <div className="hidden lg:flex items-center gap-1">
            {links.map((l) => (
              <a
                key={l.label}
                href={l.to}
                className="px-4 py-2 text-sm font-medium text-foreground/75 hover:text-foreground rounded-lg hover:bg-accent transition-all"
              >
                {l.label}
              </a>
            ))}
          ...
          <div className="hidden lg:flex items-center gap-2">
            ...
            <a href="#appointment">
              <Button size="sm" className="gradient-bg text-white shadow-glow hover:opacity-90">
                Book Now
              </Button>
            </a>
          </div>
        ...
        {open && (
          <div className="lg:hidden glass rounded-2xl mt-2 p-4 flex flex-col gap-2 animate-in fade-in slide-in-from-top-2">
            ...
            <div className="flex gap-2 pt-2 border-t">
              {user ? (
                <Link to={dashboardPath(user.role)} className="flex-1">
